...
Eerst wordt het probleemdomein geschetst: wat is de context van
toegankelijkheidsevaluaties in de Vlaamse huisartsenpraktijk, welke studies bestaan er al,
waar knelt het schoentje en voor wie wordt dit onderzoek gevoerd?
\medskip

Vervolgens wordt het technologisch landschap verkend: welke tools bestaan er vandaag voor digitale bevragingen,
hoe verhouden die zich tot de noden van dit project, en welke keuzes zijn gemaakt voor het
framework en de authenticatie?
\medskip

Ten slotte worden de succescriteria en evaluatiemethoden
...
interbeoordelaarsanalyse, test-hertest-studie en expertenbevraging resulteerden uiteindelijk in
...
ondermijnt \autocite{Merckx2023}. De test-hertest-studie bevestigde bovendien dat antwoorden
...
Het doel van deze bachelorproef kadert expliciet binnen de aanbevelingen uit het rapport Toegankelijkheid Huisartsgeneeskunde \autocite{Merckx2023} en de noden die geïdentificeerd werden binnen eerstelijnszone Aalst.
Er bestaat momenteel geen gevalideerd digitaal hulpmiddel dat specifiek afgestemd is op de toegankelijkheidsevaluaties van vlaamse huisartsen \autocite{Merckx2023}.
Deze bachelorproef wil hierop een antwoord bieden door een proof-of-concept interface te ontwikkelen die de bestaande indicatorenset omzet in een gebruiksvriendelijke, interactieve bevraging.
De interface beoogt drie concrete verbeteringen ten opzichte van de huidige vragenlijsten: een kortere invultijd, een hogere consistentie in de interpretatie van indicatoren en een lagere cognitieve belasting voor de gebruiker.
...
aangepaste navigatielogica zijn niet of slechts beperkt realiseerbaar \autocite{microsoftForms}.
\medskip
...
Door integraties met onder andere CRM- en HR-systemen is het platform breed inzetbaar, bijvoorbeeld voor klanttevredenheidsonderzoeken, medewerkerstevredenheid en marktonderzoek \autocite{surveysparrow}.
\medskip

\textbf{SurveyJS} is een open-source JavaScript-bibliotheek voor het bouwen en weergeven van interactieve vragenlijsten binnen een eigen webapplicatie. 
Anders dan de bovenstaande platforms is SurveyJS geen zelfstandig enquêteplatform, maar een bibliotheek die wordt geïntegreerd in een bestaande frontend. 
De bibliotheek biedt uitgebreide ondersteuning voor dynamische vragenlogica, conditionele weergave van vragen en een breed scala aan vraagtypen. 
Een belangrijk onderscheid met cloudgebaseerde tools is dat SurveyJS volledig client-side draait: alle data worden lokaal verwerkt en opgeslagen, zonder overdracht naar externe servers. 
SurveyJS biedt officiële integraties voor Angular, React en Vue, wat de technische inbedding in een bestaand frontend-project sterk vereenvoudigt. 
De bibliotheek is gratis beschikbaar onder een open-source licentie voor niet-commercieel gebruik \autocite{surveyjs}.
\medskip

\subsection{Kritische analyse van de bestaande tools in functie van de use case}
\label{subsec:fitgap}

Op basis van de noden die geïdentificeerd werden in de literatuur en de gesprekken met huisartsen binnen eerstelijnszone Aalst worden vijf functionele vereisten geformuleerd waaraan de ideale tool moet voldoen \autocite{Merckx2023}:

\begin{enumerate}
  \item \textbf{Dynamische vragen}: de vragenlijst past zich aan op basis van eerder gegeven antwoorden.
  \item \textbf{distributie mogelijkheden}: de tool kan worden geïntegreerd in een applicatie via embedding, een API of webhooks.
  \item \textbf{Automatische invulling}: velden kunnen vooraf worden ingevuld op basis van gekende data.
  \item \textbf{Lokale gegevensopslag}: alle data wordt lokaal opgeslagen, zonder overdracht naar externe servers. Dit is een harde vereiste in de context van de zorg, waar privacygevoelige informatie over huisartsenpraktijken verwerkt wordt.
  \item \textbf{Prijzen}: de tool is financieel haalbaar voor de beoogde context.
\end{enumerate}

Een vergelijkende analyse van de vier tools ten opzichte van deze vereisten levert het volgende beeld op:

\begin{table}[h]
  \centering
  \begin{tabularx}{\textwidth}{|X|X|X|X|X|}
    \hline
    \textbf{Criterium} & \textbf{Microsoft Forms} & \textbf{REDCap} & \textbf{SurveySparrow} & \textbf{SurveyJS} \\
    \hline
    Dynamische vragen & Beperkt & Uitstekend & Uitstekend & Uitstekend \\
    \hline
    Distributie/API & Beperkt & Goed & Uitstekend & Uitstekend\\
    \hline
    Auto-invulling & Beperkt & Ja & Ja & Ja\\
    \hline
    Lokale opslag & Nee & Ja (self-hosted) & Nee (cloud) & Ja (volledig lokaal)\\
    \hline
    Prijs & Gratis (M365) & Gratis (non-profit) & Betaald & Gratis (open-source)\\
    \hline
  \end{tabularx}
  \label{tab:fit-analyse}
  \caption{Vergelijkende analyse van de onderzochte tools 
  (op basis van \textcite{surveyjs},
  \textcite{Harris2009},
  \textcite{microsoftForms} en
  \textcite{surveysparrow})}
\end{table}

Uit deze analyse blijkt dat geen van de drie cloudgebaseerde tools volledig voldoet aan alle vereisten. 
Microsoft Forms is voor de beoogde use case onvoldoende geschikt: 
de beperkte dynamische vragenlogica vormt een kritisch obstakel. 
Er ontbreekt native API-ondersteuning en data worden opgeslagen op de Microsoft-servers, 
wat strijdig is met de vereiste van lokale gegevensopslag in een zorgcontext.
\medskip

REDCap biedt sterke functionaliteit en kan self-hosted worden ingezet, 
maar de configuratiecomplexiteit en het vereiste serverbeheer maken het minder geschikt als basis voor een snel te realiseren proof-of-concept interface.
\medskip

SurveySparrow scoort sterk op dynamische logica en distributiemogelijkheden, 
maar slaat alle data op in de cloud, wat een onoverkomelijk privacybezwaar vormt.
\medskip

SurveyJS onderscheidt zich van de overige tools doordat het als enige voldoet aan de vereiste van lokale gegevensopslag: 
de bibliotheek draait volledig client-side, zonder enige dataoverdracht naar externe servers. 
Daarnaast biedt SurveyJS uitstekende dynamische vragenlogica en een native integratie met Angular, 
wat de ontwikkeling van de proof-of-concept interface aanzienlijk vereenvoudigt.
Hoewel SurveyJS de meeste functionele vereisten afdekt, impliceert de keuze voor een client-side bibliotheek ook een hogere ontwikkelingscomplexiteit in vergelijking met low-code alternatieven zoals Microsoft Forms. 
Deze extra implementatie-inspanning wordt echter gerechtvaardigd door de noodzaak aan volledige controle over dataopslag en integratie binnen een Angular-architectuur.
\medskip

Binnen de gehanteerde criteria blijkt dat geen enkele tool optimaal scoort op alle dimensies, waardoor de keuze noodzakelijkerwijs gebaseerd is op een afweging tussen gebruiksgemak, technische flexibiliteit en privacyvereisten.
Dit leidt tot de keuze voor een op maat ontwikkelde interface waarbij SurveyJS als kernbibliotheek wordt ingezet. 
Deze keuze is het resultaat van een bewuste afweging waarbij privacy en flexibiliteit zwaarder doorwegen dan implementatiegemak en low-code configuratie, gezien de context van gevoelige gegevensverwerking in de zorg en de nood aan integratie binnen een Angular-frontend.

\subsection{Framework}
\label{subsec:framework}

Voor de ontwikkeling van de proof-of-concept interface wordt gekozen voor \textbf{Angular},
een open-source frontend-framework ontwikkeld en onderhouden door Google \autocite{angularDocs}. Deze keuze is
om meerdere redenen gemaakt.
\medskip
...
codebase ten goede komt.
\medskip

Ten tweede beschikt Angular over sterke ingebouwde
...
met conditionele logica \autocite{angularForms}.
\medskip 

Ten derde laat de bestaande voorkennis en praktijkervaring met het
...
foutdetectie in vergelijking met puur JavaScript \autocite{typescriptDocs}. De Angular CLI faciliteert de opzet en het
beheer van het project, inclusief het genereren van componenten, services en testbestanden\autocite{angularCli}.
\medskip

Hoewel Angular een uitgebreide en gestructureerde oplossing biedt, impliceert deze architectuur ook een zekere complexiteit. 
Voor een relatief kleinschalige proof-of-concept kan dit leiden tot een hogere instapdrempel en extra configuratie-overhead in vergelijking met lichtere frameworks zoals React of Vue. 
De keuze voor Angular wordt daarom niet gemaakt op basis van simpliciteit, maar op basis van de nood aan een strikt gestructureerde codebase en ingebouwde ondersteuning voor complexe formulierlogica.
...
De SUS wordt als primaire usability-meting gehanteerd om twee redenen. 
Ten eerste is het een internationaal gevalideerd instrument.
Ten tweede vereist de SUS slechts een kleine steekproef (10 tot 12 gebruikers volstaan voor betrouwbare resultaten) \autocite{Lewis,Tullis2006}.
Wat haalbaar is binnen de context van dit onderzoek.
\medskip
...
als \textit{excellent} worden aangemerkt.
\medskip
...
logging-infrastructuur moet worden opgezet \autocite{clarityDocs,hotjarDocs}.
\medskip
...
significante zones van verwarring. De heatmapdata worden gebruikt als aanvulling op de kwantitatieve
tijdsmetingen.
...
Aanvullend word de heatmapdata gebruikt om specifieke knelpunten in de interface te identificeren en te verklaren waarom bepaalde ontwerpkeuzes al dan niet bijdragen aan de beoogde verbeteringen.